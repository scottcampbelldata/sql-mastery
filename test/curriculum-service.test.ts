...
// Tests run from the project root; resolve content there so the compiled test in
// dist/test still points at the real content/ directory.
...
test('buildCurriculum extracts the first fundamentals exercise with database and expected SQL', () => {
  const curriculum = buildCurriculum({ rootDir });
  const exercise = curriculum.exercises.find((item: any) => item.id === 'p1-1');
...
test('buildCurriculum never schedules an empty session', () => {
  ...
  const emptySessions = curriculum.sessions.filter((session: any) => session.exerciseIds.length === 0);
...
test('buildCurriculum starts from absolute zero and carries teaching metadata', () => {
  const curriculum = buildCurriculum({ rootDir });
  const firstExercise = curriculum.exercises.find((item: any) => item.id === 'zero-database-01');
  const seniorCase = curriculum.exercises.find((item: any) => item.id === 'senior-analytics-case-01');
...
test('buildCurriculum keeps generated window sort columns available in outer queries', () => {
  const curriculum = buildCurriculum({ rootDir });
  const exercise = curriculum.exercises.find((item: any) => item.id === 'academy-rank-by-group-08-stackoverflow-users');
...
test('buildCurriculum includes senior interview expansion packs', () => {
  const curriculum = buildCurriculum({ rootDir });
  const advanced = curriculum.exercises.find((item: any) => item.id === 'senior-window-01');
  const caseStudy = curriculum.exercises.find((item: any) => item.id === 'case-executive-sales-01');
...
test('buildCurriculum adapts StackOverflow reference SQL to the local database port', () => {
  ...
  const stackoverflowSql = curriculum.exercises
    .filter((item: any) => item.database === 'stackoverflow' && item.expectedSql)
    .map((item: any) => item.expectedSql)
    .join('\n');
...
test('buildCurriculum preserves less-than operators from HTML solution snippets', () => {
  const curriculum = buildCurriculum({ rootDir });
  const byId = new Map(curriculum.exercises.map((item: any) => [item.id, item]));
