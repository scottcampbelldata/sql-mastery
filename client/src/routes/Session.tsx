...
import { ExerciseRail } from './session/ExerciseRail';
import { Workbench } from './session/Workbench';
...
export default function Session() {
  ...
  // App's <Body> gates this route on a loaded curriculum, so a missing one here is only
  // the transient case; narrow both curriculum and session for the rest of the render.
  if (!curriculum || !session) return <AppShell breadcrumb={<span className="here">Session</span>}><EmptyState title="Session not found" /></AppShell>;
  ...
  const goTo = (exId: string) => navigate(`/session/${session.id}/${exId}`);
  const nextTarget = ((): NextTarget | null => {
    ...
    const nextSession = session.sequence != null ? curriculum.sessions[session.sequence] : undefined; // sequence is 1-based
