...
// The learn content lives on loosely-shaped exercise fields (concept, steps,
// commonMistakes, interviewAngle, …) that different sources populate; index it broadly.
type LearnExercise = Exercise & Record<string, any>;
...
export function LearnAccordion({ exercise, defaultOpen }: Props) {
        ...
        <div className="learn-body">
          ...
          {(exercise.steps || []).length ? (
            <div className="learn-block"><span>How to approach it</span>
              <ol>{exercise.steps.map((s: string, i: number) => <li key={i}>{s}</li>)}</ol></div>) : null}
          {(exercise.commonMistakes || []).length ? (
            <div className="learn-block warn"><span>Common mistakes</span>
              <ul>{exercise.commonMistakes.map((s: string, i: number) => <li key={i}>{s}</li>)}</ul></div>) : null}
