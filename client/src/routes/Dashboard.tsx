...
import { Button, Pill, ProgressMeter, EmptyState } from '../components/ui';
import { percent, completedCount, sessionComplete, currentSession } from '../lib/curriculum';
...
export default function Dashboard() {
  ...
  // App's <Body> only renders this route once the curriculum has loaded; this guard
  // narrows the type for the rest of the component.
  ...
  const queue = curriculum.sessions.filter((s: Session) => !sessionComplete(s, progress.completed));
    ...
    <AppShell breadcrumb={<span className="here">Dashboard</span>}>
      ...
      <section id="weeks" className="dash-section">
        ...
        <div className="week-grid">
          {curriculum.weeks.map((week: Week) => {
            const sessions = week.sessions.map((id) => curriculum.sessions.find((s: Session) => s.id === id));
