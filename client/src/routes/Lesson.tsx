...
import { rewriteLessonLinks, splitLessonSections } from './lessonUtils';
import { isLessonBoxChecked, setLessonBox } from '../lib/progress';
import { useCurriculum } from '../state/CurriculumContext';
import { DrillModal } from '../components/DrillModal';
import type { Exercise } from '../types';
import './lesson.css';

const fragments = import.meta.glob('../lessons/fragments/*.html', { query: '?raw', import: 'default', eager: true }) as Record<string, string>;
...
export default function Lesson() {
  ...
  const exerciseById = useMemo(() => {
    ...
    (curriculum?.exercises || []).forEach((e: Exercise) => { map[e.id] = e; });
    return map;
  }, [curriculum]);
  ...
  const buildExercise = useCallback((problem: Element): Exercise => {
    const dataId = problem.querySelector<HTMLInputElement>('input.done')?.dataset.id;
    const fromCurriculum = dataId ? exerciseById[dataId] : undefined;
  ...
  useEffect(() => {
    ...
    if (!section) return;
    root.querySelectorAll<HTMLInputElement>('input.done[data-id]').forEach((cb) => {
      const id = cb.dataset.id;
      if (id == null) return;
      cb.checked = isLessonBoxChecked(meta.page, id);
      cb.onchange = () => setLessonBox(meta.page, id, cb.checked);
    });
  ...
  const sectionProgress = (sec: { html: string }) => {
  ...
  function markSolved(id: string) {
    ...
    const cb = bodyRef.current?.querySelector<HTMLInputElement>(`input.done[data-id="${id}"]`);
