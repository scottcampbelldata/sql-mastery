...
import { OutputDock } from '../routes/session/OutputDock.jsx';
import { Button, Callout } from './ui.jsx';
...
export function DrillModal({ exercise, onClose, onSolved }) {
  const schema = useDbSchema(exercise.database);
  const check = useSqlCheck(exercise, {
    onResult: (correct) => { if (correct) onSolved?.(exercise.id); }
  });
