...
function academyExercise(input: any) {
...
function labelForSource(source: any) {
  return `${source.database} ${source.table}`;
}

function columnsWithGroup(source: any) {
  const columns = source.columns.split(',').map((column: string) => column.trim().toLowerCase());
  return columns.includes(source.groupColumn.toLowerCase())
    ? source.columns
    : `${source.groupColumn}, ${source.columns}`;
}

function withTeaching(source: any, variant: any, extra: any) {
...
const variants: any[] = [
  ...
  },
  {
    ...
    sql: (s: any) => `SELECT COUNT(*) AS row_count\nFROM ${s.table};`
  },
  {
    suffix: 'null-audit',
    moduleId: 'm4',
  ...
  },
  {
    ...
    sql: (s: any) => `EXPLAIN SELECT ${s.columns}\nFROM ${s.table}\nWHERE ${s.measureColumn} IS NOT NULL\nORDER BY ${s.orderBy}\nLIMIT 25;`
  },
  {
...
function generatedExercise(source: any, variant: any, sourceIndex: number, variantIndex: number) {
