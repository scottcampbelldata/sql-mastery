...
import { getAcademyExpansionExercises } from './academy-expansion';
import { getInterviewExpansionExercises } from './interview-expansion';
...
function adaptExerciseToLocalDatabase(exercise: any) {
...
function parseExercise(piece: string, moduleInfo: any, index: number) {
...
function parseModule(rootDir: string, moduleInfo: any) {
...
function schemaExercise(input: any) {
  return {
...
function chunkForSession(exercises: any[], sessionIndex: number, sessionCount: number) {
...
function sessionTitle(moduleInfo: any, sessionNumber: number, sessionCount: number) {
...
function buildSessions(exercises: any[]) {
  const sessions: any[] = [];
  let globalIndex = 0;
...
function buildCurriculum(options: any = {}) {
  // Resolve content relative to the project root (process.cwd()) because the
  // server is always started from the root, while this file compiles into dist/.
  const rootDir = options.rootDir || path.resolve(process.cwd(), 'content');
  const exercises = [
