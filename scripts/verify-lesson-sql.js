...
const DB_REAL = { chinook: 'chinook_serial', stackoverflow: 'stackoverflow_dba' };
...
function stripTags(s) { return s.replace(/<\/?[a-zA-Z!][^>]*>/g, ''); }
...
function runOne(db, stmt) {
  ...
  try {
    ...
    const timedOut = /statement timeout|canceling statement/i.test(err) || e.killed;
