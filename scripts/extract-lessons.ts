...
// Resolve repo paths from the working directory (the build runs from the project
// root) so the compiled script in dist/scripts still reads content/ and writes the
// client fragments at the real project root.
const contentDir = path.join(process.cwd(), 'content');
const outDir = path.join(process.cwd(), 'client', 'src', 'lessons', 'fragments');
fs.mkdirSync(outDir, { recursive: true });

const files = fs.readdirSync(contentDir).filter((f: string) => f.endsWith('.html'));
